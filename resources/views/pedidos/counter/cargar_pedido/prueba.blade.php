...
<head>
    ...
    <script>
        ...
        // Initialize Pusher
        var pusher = new Pusher('fd05fe1046e6d4b90d0a', {
            cluster: 'sa1'
        });
        ...
        channel.bind('test.notification', function(data) {
            ...
            if (data.orderId && data.estado_entrega && data.detalles_motorizado) {
                toastr.info(
                    `<div class="notification-content">
                    <i class="fas fa-book" style="margin-left: 20px;"></i> <span>  ${data.nombre_usuario}</span>
                        <i class="fas fa-user"></i> <span>   ${data.estado_entrega}</span>
                        <i class="fas fa-book" style="margin-left: 20px;"></i> <span>  ${data.detalles_motorizado}</span>
                    </div>`,
                    `${data.orderId}`,
                    {
                        closeButton: true,
