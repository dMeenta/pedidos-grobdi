...
<div class="modal fade" id="modalEditar{{ $item->articulo_id }}" tabindex="-1" role="dialog" aria-labelledby="modalLabel{{ $item->articulo_id }}" aria-hidden="true">
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            <form action="{{ route('util.update', $item->articulo_id) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="modal-header">
                    <h1 class="modal-title" id="modalLabel{{ $item->articulo_id }}"><i class="fa-solid fa-tape"></i> Editar Útiles</h1>
                ...
                <div class="modal-body" style="max-height: 60vh; overflow-y: auto;">
                    ...
                    @if($item->articulo->estado === 'inactivo')
                        <div class="form-group">
                            <label for="estado">Estado del Útil</label>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn_crear"><i class="fa-solid fa-floppy-disk"></i> Actualizar</button>
                </div>
            </form>
        </div>
    </div>
